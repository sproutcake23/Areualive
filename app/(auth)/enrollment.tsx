...
export default function Enrollment() {
  ...
  // 🎛️ 1. Dynamic Hardware Configuration Toggles
  const [cameraPosition, setCameraPosition] = useState<'front' | 'back'>('front');
  const [isFlashOn, setIsFlashOn] = useState(false);

  const faceDetector = useFaceDetector({ performanceMode: "accurate", landmarkMode: "all" });
  const { resize } = useResizePlugin();
  ...
  // 💡 2. Dynamic Window-Level Front Camera Softbox Engine
  useEffect(() => {
    async function runSoftboxLightingEngine() {
      try {
        if (busy && cameraPosition === "front" && isFlashOn) {
  ...
  const frameProcessor = useFrameProcessor((frame) => {
    ...
    try {
      const result = enrollFaceFrame({ 
        ...
        faceDetectorPlugin: faceDetector,
        boxedMobileFaceInterpreter: boxedMobileFaceModel,
        // 🎯 Pass states to background worklet execution context securely
  ...
  }, [faceDetector, resize, shouldCapture, isProcessing, boxedMobileFaceModel, cameraPosition, isFlashOn]); 
  ...
  return (
    <View className="flex-1 bg-black">
      ...
      <View className="absolute inset-0 z-10">
        {defaultDevice && (
          <TypedCameraPreview 
            ...
            // ⚡ BACK CAMERA LIGHTING CONTROL: Real torch activation for rear matrix
            torch={cameraPosition === "back" && isFlashOn && busy ? "on" : "off"}
          />
        )}
        
        {/* Hide standard overlay during softbox window flash */}
        {!(busy && cameraPosition === "front" && isFlashOn) && <FaceOverlay />}
      </View>

      {/* =============================================================================
          💡 FRONT CAMERA FULL-SCREEN SOFTBOX FLASH MASK (Option 2)
          ============================================================================= */}
      {busy && cameraPosition === "front" && isFlashOn && (
        <View pointerEvents="none" className="absolute inset-0 z-20 bg-white flex justify-center items-center">
          ...
          <Text className="text-slate-800 font-bold tracking-widest text-sm absolute top-24">LOCKING VECTOR PROFILE...</Text>
        </View>
      )}
      ...
      {/* Diagnostic Crop Output View Panel */}
      {enrollmentPreview && (
        <View className="absolute top-36 right-6 z-50 border-2 border-yellow-500 rounded-2xl overflow-hidden shadow-2xl bg-slate-900 p-2">
          <Text className="text-[10px] text-yellow-500 font-bold text-center mb-1">ENROLLMENT MATRIX VIEW</Text>
          <Image source={{ uri: enrollmentPreview }} className="w-28 h-28 rounded-xl bg-black" resizeMode="contain" />
      ...
      <View className="absolute left-0 right-0 z-40 px-6" style={{ top: Math.max(insets.top, 16) + 8 }} pointerEvents="box-none">
        <TouchableOpacity onPress={() => { if (!busy) { setShowCamera(false); setName(""); setErrorMessage(null); } }} disabled={busy} activeOpacity={0.7} className="mb-4 w-10">
          <MaterialCommunityIcons name="arrow-left" size={28} color="white" />
        </TouchableOpacity>
        <Text className="text-2xl font-semibold text-white shadow-sm">Face registration</Text>
      </View>
