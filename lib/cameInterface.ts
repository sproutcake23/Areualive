...
export function verifyFaceFrame(input: FaceVerificationInput): FaceVerificationResult {
  ...
  const { frame, enrolledFaceDescriptor, currentChallenge, resizePlugin, faceDetectorPlugin, boxedMobileFaceInterpreter , boxedAntiSpoofInterpreter, cameraPosition, isFlashOn} = input;
  ...
  try {
    ...
    let correctRotation = "0deg";
      if (cameraPosition === "front") {
        ...
        // Rear cameras require an alternate orientation alignment to remain upright!
    ...
    const croppedFaceNetBuffer = resizePlugin(frame, {
      ...
      pixelFormat: 'rgb',
      dataType: 'float32',
      rotation: correctRotation, // 🔄 FIX: Rotated 180 degrees back to upright display orientation
    ...
    const MiniFasModel = boxedAntiSpoofInterpreter.unbox() as TensorflowModel;
    const ProbabilityOutput = MiniFasModel.runSync([faceNetFloatArray.buffer]);

    const probability = Array.from(new Float32Array(ProbabilityOutput[0])) as number[]; 

    if(probability[1] < 0.7) {
      console.log("It is a fake image", probability[1])
      return { isMatch: false, livenessConfirmed: false, confidence: 0, livenessStep: currentChallenge };
    }
    ...
    console.log("It is a real image", probability[1])
    console.log(
    ...
    console.log(
      ...
      `\n  └─ Final Authentication Verdict:  ${similarityScore >= 0.35 ? "🎉 AUTHORIZED MATCH" : "🔒 ACCESS DENIED"}`
    );
...
export function enrollFaceFrame(input: EnrollmentInput): EnrollmentResult {
  ...
  try {
    ...
    const croppedFaceNetBuffer = resizePlugin(frame, {
      ...
      pixelFormat: 'rgb',
      dataType: 'float32',
      rotation: isPortrait ? "0deg" : "90deg", // 🔄 FIX: Rotated 180 degrees back to upright display orientation
      mirror: true
    });
